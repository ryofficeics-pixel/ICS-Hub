...
- `src/main.js`: rendering, search, filters, launch actions, recent tracking
- `src/tools-data.js`: source of truth for tool cards and routing URLs
...
The future auto-routing agent should use this hub as the central directory for operational tools. The agent can read or mirror `src/tools-data.js` as a registry, then route users based on:
...
7. Track routing events in the agent layer, not in this static hub.

## Multi-Account Routing Considerations

This static hub does not own account data. Multi-account routing should live in a separate agent/service layer.
...
- Static hub: public directory and launcher UI
...
5. Keep the static hub independent from auth/database code unless a future requirement explicitly changes the architecture.
