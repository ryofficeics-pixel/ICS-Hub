...
alter table public.tools
  add column if not exists required_role text check (required_role in ('super_admin', 'admin', 'supervisor', 'staff', 'viewer'));
...
create index if not exists idx_daily_reports_project_date_v2 on public.daily_reports(project_id, report_date desc);
create index if not exists idx_weekly_reports_project_week_v2 on public.weekly_reports(project_id, week_start desc);
create index if not exists idx_survey_reports_project_date_v2 on public.survey_reports(project_id, survey_date desc);
...
using (coalesce(user_id, id) = auth.uid() or public.is_admin_or_higher());
...
using (coalesce(user_id, id) = auth.uid() or public.is_super_admin())
with check (coalesce(user_id, id) = auth.uid() or public.is_super_admin());
